...
function updateCart(key, quantity, event) {
	...
	const qnty = Number($('input[name=\'' + key + '\']').val());

	$.ajax({
		url: 'index.php?route=checkout/cart/editForCart',
		type: 'post',
		data: 'quantity[' + key + ']=' + (typeof (qnty) != 'undefined' ? qnty : 1),
		dataType: 'json',
		beforeSend: function () { },
		complete: function () { },
...
var cart = {
	...
	// },
	'add': function (product_id, quantity) {
		const options = $('#product_' + product_id + ' input[type=\'radio\']:checked');
		const data = options.serialize() + '&product_id=' + product_id + '&quantity=' + (typeof (quantity) != 'undefined' ? quantity : 1);
